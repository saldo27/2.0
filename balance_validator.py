...
class BalanceValidator:
    ...
    def validate_schedule_balance(self, schedule: dict, workers_data: list[dict]) -> dict:
        ...
        logging.info("📊 Balance Validation Summary (Phase System):")
        logging.info(
            f"   Phase 1 target (≤{self.tolerance_percentage}%): {len(violations['within_tolerance'])} workers"
        )
        logging.info(
            f"   Phase 2 range ({self.tolerance_percentage}%-{self.emergency_limit}%): {len(violations['within_emergency'])} workers"
        )
        logging.info(
            f"   CRITICAL - Beyond absolute limit (>{self.emergency_limit}%): {len(violations['critical'])} workers"
        )
    ...
    def get_rebalancing_recommendations(self, schedule: dict, workers_data: list[dict]) -> list[dict]:
        ...
        if recommendations:
            logging.info("💡 Top rebalancing recommendations:")
            for i, rec in enumerate(recommendations[:5], 1):
                logging.info(
                    f"   {i}. Transfer {rec['shifts_to_transfer']} shifts from "
    ...
    def check_transfer_validity(
        self, from_worker_id: str, to_worker_id: str, schedule: dict, workers_data: list[dict]
    ) -> tuple[bool, str]:
        """
        ...
        if from_improves and to_improves:
            ...
            return (
                False,
                f"Transfer would worsen balance (from: {from_deviation:.1f}%→{from_deviation_after:.1f}%, to: {to_deviation:.1f}%→{to_deviation_after:.1f}%)",
            )

    def validate_bridge_balance(self, scheduler, bridge_periods: list[dict], tolerance: float = 0.5) -> dict:
        """
        ...
        for worker in scheduler.workers_data:
            ...
            if abs_deviation <= tolerance:
                ...
                logging.warning(
                    f"⚠️  Worker {worker_id}: bridge deviation {deviation:+.2f} "
        ...
        logging.info("Bridge Balance Validation Results:")
        ...
        if exceeds_count > 0:
            ...
            for worker_info in violations["exceeds_tolerance"][:5]:  # Show first 5
                logging.warning(
                    f"     {worker_info['worker_id']}: "
                    f"{worker_info['assigned']} assigned vs {worker_info['target']:.2f} target "
                    f"(deviation: {worker_info['deviation']:+.2f})"
                )

        return {"violations": violations, "stats": stats, "is_balanced": exceeds_count == 0}

    def get_bridge_rebalancing_recommendations(
        self, scheduler, bridge_periods: list[dict], tolerance: float = 0.5
    ) -> list[dict]:
        """
        ...
        if recommendations:
            logging.info("💡 Top bridge rebalancing recommendations:")
            for i, rec in enumerate(recommendations[:5], 1):
                logging.info(
                    f"   {i}. Transfer {rec['bridges_to_transfer']} bridge(s) from "
