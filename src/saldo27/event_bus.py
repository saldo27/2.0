...
class EventBus:
    ...
    def emit(self, event_type: EventType, user_id: str | None = None, **data) -> None:
        """
        ...
        """
        event = ScheduleEvent(event_type=event_type, user_id=user_id, data=data)
        self.publish(event)

    def get_event_history(
        self, event_type: EventType | None = None, since: datetime | None = None, limit: int | None = None
    ) -> list[ScheduleEvent]:
        """
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        with self._lock:
            ...
            listener_counts = {event_type.value: len(listeners) for event_type, listeners in self._listeners.items()}
