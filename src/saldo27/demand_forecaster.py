...
from saldo27.performance_cache import cached, monitor_performance, time_function
...
class DemandForecaster:
    ...
    def __init__(self, historical_data_manager=None):
        ...
        self.config = {
            ...
            "ensemble_weights": {"arima": 0.3, "seasonal": 0.3, "random_forest": 0.4},
        }
    ...
    def _generate_time_series_forecast_cached(self, data: dict[str, Any], forecast_days: int) -> dict[str, Any] | None:
        """Generate ARIMA-based time series forecasts (cached)"""
        return self._generate_time_series_forecast(data, forecast_days)

    @cached(ttl=1800)  # Cache for 30 minutes
    def _generate_seasonal_forecast_cached(self, data: dict[str, Any], forecast_days: int) -> dict[str, Any] | None:
        """Generate forecasts using seasonal decomposition (cached)"""
        return self._generate_seasonal_forecast(data, forecast_days)

    @cached(ttl=1800)  # Cache for 30 minutes
    def _generate_ml_forecast_cached(self, data: dict[str, Any], forecast_days: int) -> dict[str, Any] | None:
        """Generate forecasts using machine learning models (cached)"""
        return self._generate_ml_forecast(data, forecast_days)

    def _generate_time_series_forecast(self, data: dict[str, Any], forecast_days: int) -> dict[str, Any] | None:
        ...
        try:
            ...
            # Generate forecasts
            forecast_dates = pd.date_range(start=timestamps.max() + timedelta(days=1), periods=forecast_days, freq="D")
    ...
    def _generate_seasonal_forecast(self, data: dict[str, Any], forecast_days: int) -> dict[str, Any] | None:
        ...
        try:
            ...
            if len(trend) >= 2:
                ...
                forecast_dates = pd.date_range(
                    start=timestamps.max() + timedelta(days=1), periods=forecast_days, freq="D"
                )
    ...
    def _generate_ml_forecast(self, data: dict[str, Any], forecast_days: int) -> dict[str, Any] | None:
        ...
        try:
            ...
            for i, date in enumerate(forecast_dates):
                ...
                if i == 0:
                    ...
                    forecast_row["fill_rate_lag_7"] = forecast_row.get(
                        "fill_rate_lag_1", last_known_values["fill_rate"]
                    )
                    forecast_row["efficiency_lag_7"] = forecast_row.get(
                        "efficiency_lag_1", last_known_values["efficiency"]
                    )
                    forecast_row["fill_rate_lag_14"] = last_known_values["fill_rate"]
                    forecast_row["efficiency_lag_14"] = last_known_values["efficiency"]
    ...
    def _generate_ensemble_forecast(self, forecasts: dict[str, Any], forecast_days: int) -> dict[str, Any] | None:
        ...
        try:
            ...
            if not forecast_dates:
                ...
                forecast_dates = [(start_date + timedelta(days=i)).strftime("%Y-%m-%d") for i in range(forecast_days)]

            result = {"dates": forecast_dates, "method": "Ensemble"}
    ...
    def _generate_basic_forecasts(self, forecast_days: int) -> dict[str, Any]:
        ...
        forecast_dates = [(base_date + timedelta(days=i)).strftime("%Y-%m-%d") for i in range(1, forecast_days + 1)]
