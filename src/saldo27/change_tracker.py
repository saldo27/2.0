...
class ChangeTracker:
    ...
    def mark_redo_applied(self) -> ChangeRecord | None:
    ...
    def get_audit_trail(
        ...
        if worker_id:
            filtered_changes = [c for c in filtered_changes if worker_id in c.affected_workers]

        if date:
            filtered_changes = [c for c in filtered_changes if date in c.affected_dates]
    ...
    def import_audit_data(self, data: dict[str, Any]) -> bool:
        ...
        """
        try:
            self.changes = [ChangeRecord.from_dict(change_data) for change_data in data["changes"]]
    ...
    def get_statistics(self) -> dict[str, Any]:
        ...
        """
        if not self.changes:
            return {"total_changes": 0, "can_undo": False, "can_redo": False}
