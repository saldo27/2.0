...
class IterativeOptimizer:
    ...
    def optimize_schedule(
        ...
        logging.info(
            f"🔍 DEBUG: About to start loop with range(1, {self.max_iterations} + 1) = range(1, {self.max_iterations + 1})"
        )
        logging.info(
            f"🔍 DEBUG: This should generate iterations: {list(range(1, min(self.max_iterations + 1, 6)))[:5]}..."
        )
        ...
        for iteration in range(1, self.max_iterations + 1):
            ...
            if self.weekend_only_mode:
                logging.info("   🎯 WEEKEND-ONLY MODE ACTIVE - specialized optimization in progress")
            ...
            if total_violations < best_violations:
                ...
                logging.info(
                    f"   📈 New best result: {total_violations} violations (improvement: {improvement_ratio:.2%})"
                )
                ...
                logging.warning(
                    f"   ⚠️  No change this iteration (no-change: {self.no_change_counter}/{self.max_no_change}, stagnation: {self.stagnation_counter}/{self.convergence_threshold})"
                )
                ...
                if self.no_change_counter >= effective_max_no_change:
                    logging.warning(
                        f"   🛑 Stopping: {self.no_change_counter} iterations with NO changes - system cannot improve further with current constraints"
                    )
                    break
            else:
                self.stagnation_counter += 1
                ...
                logging.info(
                    f"   📊 No improvement this iteration (stagnation: {self.stagnation_counter}/{self.convergence_threshold})"
                )
                logging.info(
                    f"   📊 No improvement this iteration (stagnation: {self.stagnation_counter}/{self.convergence_threshold})"
                )
            ...
            activate_weekend_mode = weekend_violations >= 2 and (
                weekend_percentage >= 75.0  # ≥75% son de fin de semana
                or (weekend_violations > general_violations and weekend_violations >= 2)
                or (weekend_violations >= 2 and general_violations <= 2)
            )

            if activate_weekend_mode:
                if not self.weekend_only_mode:
                    logging.info(
                        f"🎯 Activating WEEKEND-ONLY optimization mode "
                    ...
                    if self.no_change_counter > 0:
                        ...
                        logging.info("   🔄 Reset no_change_counter for WEEKEND-ONLY mode")
                ...
                if self.stagnation_counter >= 3 or self.no_change_counter >= 2:
                    if not self.relaxed_weekend_constraints:
                        ...
                        logging.info(
                            f"   🔓 RELAXED weekend constraints activated (stagnation: {self.stagnation_counter}, no_change: {self.no_change_counter})"
                        )
                        logging.info("   🔓 Allowing 2 weekend shifts per calendar week to enable redistribution")
            else:
                if self.weekend_only_mode:
                    logging.info(
                        f"🔄 Deactivating WEEKEND-ONLY mode (general: {general_violations}, weekend: {weekend_violations})"
                    )
                    self.weekend_only_mode = False
                    self.relaxed_weekend_constraints = False
                ...
                if weekend_violations > 0 and not self.weekend_only_mode:
                    logging.debug(
                        f"   ℹ️  Weekend-only NOT active: weekend={weekend_violations}, general={general_violations} "
            ...
            try:
                ...
                logging.info("   ✅ Optimization strategies applied, continuing to next iteration...")
    ...
    def _apply_optimization_strategies(
        ...
        if self.weekend_only_mode and weekend_violations:
            logging.info("   🎯 WEEKEND-ONLY MODE: Applying focused weekend optimization")
            ...
            if validator:
                ...
                general_violations = validation_report.get("general_shift_violations", [])
                logging.info(
                    f"   📊 Re-validated after redistribution: {len(general_violations)} general, {len(weekend_violations)} weekend violations"
                )
            ...
            if len(weekend_violations) >= 4 and self.stagnation_counter >= 2:
                logging.info(
                    f"   🔥 AGGRESSIVE MODE: {len(weekend_violations)} persistent violations, stagnation: {self.stagnation_counter}"
                )
                ...
                if validator:
                    fresh_weekend = self._create_validation_report(validator, optimized_schedule).get(
            ...
            if weekend_violations and len(weekend_violations) >= 2:
                logging.info(
                    f"   🔄 Applying cross-redistribution swaps for {len(weekend_violations)} weekend violations"
                )
                optimized_schedule = self._apply_weekend_swaps(
                    optimized_schedule, validation_report, workers_data, schedule_config
                )
        ...
        logging.info("   🔍 Validating balance after redistribution...")
        ...
        if not balance_result["is_balanced"]:
            ...
            if extreme_count > 0 or critical_count > 3:
                logging.error(
                    f"   🚨 Balance validation FAILED: {extreme_count} extreme, {critical_count} critical violations"
                )
    ...
    def _redistribute_general_shifts(
        ...
        try:
            ...
            if workers_data:
                ...
                logging.info(
                    f"Debug: Available keys: {list(workers_data[0].keys()) if isinstance(workers_data[0], dict) else 'Not a dict'}"
                )
        ...
        for need in need_more_shifts:
            logging.info(
                f"      🔴 {need['worker']} needs {need['shortage']} more shifts (deviation: {need['deviation']:.1f}%)"
            )
        for excess in have_excess_shifts:
            logging.info(
                f"      🔵 {excess['worker']} has {excess['excess']} excess shifts (deviation: {excess['deviation']:.1f}%)"
            )
        ...
        logging.info("   🎯 BALANCED MODE: Each removal must match with an assignment")
        ...
        for excess_info in have_excess_shifts:
            ...
            logging.info(
                f"   🔄 Processing {excess_worker} (deviation: {excess_info['deviation']:.1f}%, excess: {excess_info['excess']})"
            )
            ...
            for date_key, shift_type, workers in worker_shifts:
                ...
                if self._is_mandatory_shift(excess_worker, date_key, workers_data):
                    logging.debug(
                        f"      🔒 SKIPPING mandatory shift for {excess_worker} on {date_key} - cannot redistribute"
                    )
                    continue
                ...
                for need_info in need_more_shifts:
                    ...
                    if need_worker not in workers:
                        if self._can_worker_take_shift(
                            ...
                            candidates_blocked += 1
                            logging.debug(
                                f"         ❌ {need_worker} blocked by constraints for {shift_type} on {date_key}"
                            )

                logging.debug(
                    f"      📊 Candidates: {candidates_checked} checked, {candidates_blocked} blocked, best: {best_recipient}"
                )
                ...
                if best_recipient:
                    ...
                    current_need_deviation = next(
                        (n["abs_deviation"] for n in need_more_shifts if n["worker"] == best_recipient), 0
                    )
                    ...
                    if projected_improvement > 0.5:  # Minimum improvement threshold
                        ...
                        if isinstance(workers, list):
                            ...
                            try:
                                ...
                                if workers[idx] != best_recipient:
                                    logging.error(
                                        f"GENERAL list accounting error: post {idx} expected {best_recipient}, got {workers[idx]} — rolling back"
                                    )
                            ...
                            # Post-swap verification
                            if (
                                workers.count(excess_worker) != pre_old_count - 1
                                or workers.count(best_recipient) != pre_new_count + 1
                            ):
                                logging.error("GENERAL dict accounting error — rolling back")
                        ...
                        balance_tracker["shifts_removed"][excess_worker] = (
                            balance_tracker["shifts_removed"].get(excess_worker, 0) + 1
                        )
                        balance_tracker["shifts_added"][best_recipient] = (
                            balance_tracker["shifts_added"].get(best_recipient, 0) + 1
                        )
                        ...
                        date_display = (
                            date_key.strftime("%Y-%m-%d") if isinstance(date_key, datetime) else str(date_key)
                        )
                        logging.info(
                            f"      🔄 Moved {shift_type} from {excess_worker} to {best_recipient} on {date_display} (improvement: {projected_improvement:.1f})"
                        )
                    else:
                        failed_attempts += 1
                        logging.debug(
    ...
    def _can_worker_take_shift(
        self, worker_name: str, date_key, shift_type: str, schedule: dict, workers_data: list[dict]
    ) -> bool:
        """
        ...
        try:
            ...
            if not worker_data:
                logging.debug(
                    f"Worker data not found for {worker_name}. Available IDs: {[w.get('id') for w in workers_data]}"
                )
                return False
            ...
            for assigned_date in worker_assignments:
                ...
                if (days_between == 7 or days_between == 14) and shift_date.weekday() == assigned_date.weekday():
                    ...
                    logging.debug(
                        f"❌ {worker_name} blocked: 7/14 day pattern violation - {shift_date.strftime('%A %Y-%m-%d')} vs {assigned_date.strftime('%A %Y-%m-%d')}"
                    )
                    return False
                    ...
                    for date_entry in schedule:
                        try:
                            check_date = (
                                ...
                                else datetime.strptime(date_entry, "%Y-%m-%d")
                            )
            ...
            if is_weekend and hasattr(self, "scheduler") and self.scheduler:
                ...
                weekend_shifts_this_week = sum(
                    1 for d in weekend_dates if d - timedelta(days=d.weekday()) == current_week_start
                )
                if weekend_shifts_this_week >= max_weekend_per_week:
                    logging.debug(
                        f"❌ {worker_name} blocked: Already has {weekend_shifts_this_week} weekend shift(s) in week starting {current_week_start.strftime('%Y-%m-%d')} (max: {max_weekend_per_week})"
                    )
                    return False
                ...
                if consecutive_count > max_consecutive_weekends:
                    logging.debug(
                        f"❌ {worker_name} blocked: Consecutive weekend limit - "
                ...
                if total_schedule_days > 0 and total_weekend_days > 0 and total_target_for_weekend > 0:
                    ...
                    if current_weekend_count > max_weekend_allowed:
                        logging.debug(
                            f"❌ {worker_name} blocked: Weekend tolerance - "
            ...
            if target_shifts > 0:
                ...
                if non_mandatory_shifts + 1 > max_shifts:
                    logging.debug(
                        f"❌ {worker_name} blocked: Tolerance violation - "
    ...
    def _redistribute_weekend_shifts(
        ...
        for need in need_more_weekends:
            logging.info(
                f"      🔴 {need['worker']} needs {need['shortage']} more weekends (deviation: {need['deviation']:.1f}%)"
            )
        for excess in have_excess_weekends:
            logging.info(
                f"      🔵 {excess['worker']} has {excess['excess']} excess weekends (deviation: {excess['deviation']:.1f}%)"
            )
                ...
                if best_recipient:
                    ...
                    if isinstance(workers, list):
                        ...
                        try:
                            ...
                            if workers[idx] != best_recipient:
                                logging.error(
                                    f"WEEKEND list accounting error: post {idx} expected {best_recipient}, got {workers[idx]} — rolling back"
                                )
                                workers[idx] = pre_at_post
                                continue
                        except ValueError:
                        ...
                        if (
                            ...
                            logging.error("WEEKEND dict accounting error — rolling back")
                            workers.remove(best_recipient)
                            workers.append(excess_worker)
                            continue
                    ...
                    logging.info(
                        f"      🔄 Weekend: Moved {shift_type} from {excess_worker} to {best_recipient} on {day_name} {date_display}"
                    )
    ...
    def _apply_weekend_swaps(
        self, schedule: dict, validation_report: dict, workers_data: list[dict], schedule_config: dict
    ) -> dict:
        """Apply direct weekend shift swaps between over-assigned and under-assigned workers."""
        logging.info("   🔄 Applying weekend shift swaps for targeted balancing")
        ...
        if not weekend_violations:
            logging.info("   ℹ️ No weekend violations to swap")
            return optimized_schedule
        ...
        for over_info in over_assigned:
                        ...
                        for shift_type, workers in assignments.items():
                            if over_worker in workers:
                                over_weekend_shifts.append(
                                    {"date": date_key, "shift_type": shift_type, "workers_list": workers}
                                )
                    elif isinstance(assignments, list):
                        for post_idx, worker in enumerate(assignments):
            ...
            for under_info in under_assigned:
                ...
                if not over_weekend_shifts:
                    ...
                    logging.debug("      ⚠️ No weekend shifts found for over-assigned workers to swap")
                    continue
                ...
                for over_shift in over_weekend_shifts:
                    ...
                    if self._is_mandatory_shift(over_worker, date_key, workers_data):
                        logging.debug(
                            f"      🔒 SKIPPING mandatory shift for {over_worker} on {date_key} - cannot swap"
                        )
                        continue
                    ...
                    if self._can_worker_take_shift(
                        ...
                        if isinstance(workers_list, list):
                            ...
                            if (
                                workers_list.count(over_worker) != pre_old_count - 1
                                or workers_list.count(under_worker) != pre_new_count + 1
                            ):
                                logging.error("SWAP dict accounting error — rolling back")
        ...
        if swaps_made == 0 and attempts > 0:
            ...
            logging.warning("      💡 TIP: Constraints may be too strict (gap_between_shifts, 7/14 rule)")
    ...
    # ------------------------------------------------------------------
    def _apply_weekend_weekday_rotation(
        self, schedule: dict, validation_report: dict, workers_data: list[dict], schedule_config: dict
    ) -> dict:
        ...
        over_assigned = sorted(
            ...
            key=lambda v: abs(v.get("deviation_percentage", 0)) * random.uniform(0.7, 1.3),
            reverse=True,
        )
            ...
            for under_info in under_assigned:
                ...
                for we_date, we_idx in over_we_shifts:
                    ...
                    for wd_date, wd_idx in under_wd_shifts:
                        ...
                        if (
                            optimized_schedule[we_date][we_idx] != under_worker
                            or optimized_schedule[wd_date][wd_idx] != over_worker
                        ):
                            logging.error("ROTATION accounting error — rolling back both slots")
    ...
    # ------------------------------------------------------------------
    def _apply_chain_weekend_rotation(
        self, schedule: dict, validation_report: dict, workers_data: list[dict], schedule_config: dict
    ) -> dict:
                    ...
                    for mediator in worker_names:
                        ...
                        for d_med in weekend_dates:
                            ...
                            for med_idx, med_w in enumerate(med_assignments):
                                ...
                                if (
                                    optimized_schedule[d_over][idx_over] != mediator
                                    or optimized_schedule[d_med][med_idx] != under_worker
                                ):
                                    logging.error("CHAIN accounting error — rolling back both slots")
            ...
            for under_info in under_assigned:
                ...
                for we_date, we_idx in over_we_shifts:
                    ...
                    if self._can_worker_take_shift(
                        ...
                        under_info["shortage"] = under_info.get("shortage", 1) - 1
                        logging.info(
                            f"      ⛓️  DIRECT: {over_worker}→{under_worker} on "
                    ...
                    for cd_key, cd_idx in conflict_dates:
                        ...
                        for z_worker in candidates:
                            ...
                            if not self._can_worker_take_shift(
                                z_worker, cd_key, cd_shift_type, optimized_schedule, workers_data
                            ):
                                continue
    ...
    def _apply_random_perturbations(
        self, schedule: dict, workers_data: list[dict], schedule_config: dict, intensity: float = 0.1
    ) -> dict:
        ...
        logging.info(
            f"   🎲 Total assignments: {total_assignments}, planned SA swaps: {num_swaps} (stagnation boost: {_stag})"
        )
            ...
            if isinstance(_asgn, dict):
                for _v in _asgn.values():
                    if isinstance(_v, list):
                        for _wn in _v:
                            if _wn:
                                _sa_monthly.setdefault(_wn, {})[_ym] = _sa_monthly.setdefault(_wn, {}).get(_ym, 0) + 1
        ...
        if _sa_weekend_mode:
            ...
            logging.info(
                f"   🎲 SA weekend-bias: {len(_sa_weekend_dates)} weekend, {len(_sa_weekday_dates)} weekday dates"
            )
        ...
        for swap_attempt in range(num_swaps):
            ...
            if not self._can_worker_take_shift(new_worker, random_date, random_shift, optimized_schedule, workers_data):
            ...
            if accept:
                if isinstance(assignments, dict):
                    ...
                    if pre_old_in_slot == 0:
                        logging.warning(f"SA dict: {old_worker} missing from slot on {random_date} - cancelling swap")
                        T *= cooling_rate
                        continue
                    ...
                    if post_old_in_slot != pre_old_in_slot - 1 or post_new_in_slot != pre_new_in_slot + 1:
                        logging.error(
    ...
    def _apply_forced_redistribution(
        self, schedule: dict, violations: list[dict], workers_data: list[dict], schedule_config: dict
    ) -> dict:
        """
        ...
        if len(violations) > 20:
            ...
            logging.warning(
                f"⚠️ EXTREME FORCED MODE: {len(violations)} violations, allowing {max_forced} forced redistributions"
            )
        else:
        ...
        for violation in general_violations:
            ...
            if abs(deviation) > 8:  # Reduced from 15 - ANY violation beyond tolerance needs fixing
                ...
                _g9_worker_data = next(
                    (w for w in workers_data if w.get("id") == worker or f"Worker {w.get('id')}" == worker), None
                )
                    ...
                    if format_type == "dict":
                        ...
                        for candidate in worker_names:
                            if candidate != worker:
                                ...
                                if self._can_worker_take_shift(
                                    candidate, date_key_try, shift_type_try, optimized_schedule, workers_data
                                ):
                                    ...
                                    candidate_data = next(
                                        (
                                            w
                                            for w in workers_data
                                            if w.get("id") == candidate or f"Worker {w.get('id')}" == candidate
                                        ),
                                        None,
                                    )
                                    if candidate_data:
                                        ...
                                        current = self._count_worker_shifts(
                                            candidate, optimized_schedule, workers_data, exclude_mandatory=True
                                        )
                        ...
                        if valid_alternatives_with_priority:
                            ...
                            if pre_worker_in_slot == 0:
                                logging.warning(
                                    f"FORCED dict: {worker} not in slot {shift_type_try} on {date_key_try} — skipping"
                                )
                                continue
                            ...
                            if post_worker_in_slot != pre_worker_in_slot - 1 or post_alt_in_slot != pre_alt_in_slot + 1:
                                logging.error(
                ...
                if redistributed_count > 0:
                    ...
                    logging.warning(
                        f"      ⚠️ Could not redistribute any shifts from {worker} - all blocked by constraints"
                    )
    ...
    def get_optimization_summary(self) -> dict:
        ...
        return {
            ...
            "improvement": self.optimization_history[0]["total_violations"]
            - self.optimization_history[-1]["total_violations"],
    ...
    def _count_worker_shifts(
        ...
        if exclude_mandatory and workers_data:
            worker_data = next(
                (w for w in workers_data if w.get("id") == worker_name or f"Worker {w.get('id')}" == worker_name), None
            )
        ...
        try:
            for date_key, assignments in schedule.items():
                ...
                if exclude_mandatory and mandatory_dates_str:
                    try:
                        check_date = (
                            date_key if isinstance(date_key, datetime) else datetime.strptime(date_key, "%Y-%m-%d")
                        )
    ...
    def _create_validation_report(self, validator, current_schedule: dict) -> dict:
        ...
        try:
            ...
            return {"general_shift_violations": [], "weekend_shift_violations": [], "total_violations": 0}
    ...
    def _greedy_fill_empty_slots(
        ...
        """
        logging.info("   🎯 GREEDY FILL: Starting empty slot filling")
        ...
        try:
            ...
            for date, assignments in optimized_schedule.items():
                if isinstance(assignments, list):
                    for post_idx, worker in enumerate(assignments):
                        if worker is None:
                            empty_slots.append({"date": date, "post": post_idx, "format": "list"})
                elif isinstance(assignments, dict):
                    for shift_type, shift_workers in assignments.items():
                        if isinstance(shift_workers, list):
                            for idx, worker in enumerate(shift_workers):
                                if worker is None:
                                    empty_slots.append(
                                        {"date": date, "shift_type": shift_type, "idx": idx, "format": "dict"}
                                    )

            if not empty_slots:
                logging.info("   ✅ No empty slots found")
                return optimized_schedule
            ...
            for slot in empty_slots:
                ...
                for worker in workers_data:
                    ...
                    if self._can_worker_take_shift(worker_name, date, shift_type, optimized_schedule, workers_data):
                ...
                if slot["format"] == "list":
                    ...
                    if optimized_schedule[date][slot["post"]] != best_worker["worker_name"]:
                        logging.error("GREEDY list verification failed — rolling back")
                    ...
                    if optimized_schedule[date][slot["shift_type"]][slot["idx"]] != best_worker["worker_name"]:
                        logging.error("GREEDY dict verification failed — rolling back")
                ...
                if filled_count <= 5:  # Log first 5 assignments
                    ...
                    logging.info(
                        f"      ✅ Filled slot on {date_str}: {best_worker['worker_name']} "
    ...
    def _can_worker_take_greedy_shift(
        self, worker_name: str, worker_id, date, slot: dict, schedule: dict, workers_data: list[dict], scheduler_core
    ) -> bool:
        """
            ...
            if hasattr(scheduler_core, "builder") and scheduler_core.builder:
                ...
                if worker_data:
                        ...
                        try:
                            ...
                            for d, assigns in schedule.items():
                                try:
                                    ...
                                    date_str = check_date.strftime("%d-%m-%Y")
                                    if (
                                        date_str in mandatory_parts
                                        or check_date.strftime("%Y-%m-%d") in mandatory_parts
                                    ):
                                        if isinstance(assigns, list) and worker_name in assigns:
                                            mandatory_count += 1
                                        elif isinstance(assigns, dict):
                    ...
                    if target_shifts > 0:
                        ...
                        if non_mandatory_shifts + 1 > max_shifts:
                            logging.debug(
                                f"   ❌ Tolerance violation prevented: {worker_name} "
                                f"would have {non_mandatory_shifts + 1}/{target_shifts} shifts "
