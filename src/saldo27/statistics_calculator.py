...
from saldo27.performance_cache import cached, memoize, monitor_performance, time_function
...
class StatisticsCalculator:
    ...
    def _analyze_gaps(self, worker_id):
        ...
        gaps = [(sorted_assignments[i + 1] - sorted_assignments[i]).days for i in range(len(sorted_assignments) - 1)]
    ...
    def _analyze_monthly_balance(self):
        ...
        all_months = {
            f"{date.year}-{date.month:02d}" for assignments in self.worker_assignments.values() for date in assignments
        }

        # Pre-compute monthly distributions for all workers
        worker_monthly_distributions = {
            worker_id: self._get_monthly_distribution(worker_id) for worker_id in self.worker_assignments
        }

        for month in sorted(all_months):
            # Use list comprehension for efficient data collection
            worker_counts = [worker_dist.get(month, 0) for worker_dist in worker_monthly_distributions.values()]
    ...
    def _calculate_post_rotation_coverage(self):
        ...
        return {"overall_score": total_score, "worker_scores": worker_scores}
    ...
    def _calculate_coverage(self):
        ...
        actual_shifts = sum(sum(1 for w in shifts if w is not None) for shifts in self.schedule.values())
    ...
    def _count_constraint_violations(self):
        ...
        return {
            "gap_violations": sum(len(skips["gap"]) for skips in self.constraint_skips.values()),
            "incompatibility_violations": sum(
                len(skips["incompatibility"]) for skips in self.constraint_skips.values()
            ),
            "reduced_gap_violations": sum(len(skips["reduced_gap"]) for skips in self.constraint_skips.values()),
        }
    ...
    def calculate_statistics(self):
        ...
        """
        stats = {"workers": {}, "schedule": {}, "coverage": 0.0}
        ...
        # TEMPORARILY DISABLED: Add bridge statistics if bridges are detected
        # Commenting out to test if this is causing counting issues
        """
        if hasattr(self.scheduler, 'data_manager') and hasattr(self.scheduler.data_manager, 'bridge_manager'):
            ...
            for worker_id in stats['workers']:
                ...
                stats['workers'][worker_id]['bridge_balance'] = bridge_stats['bridge_balance'].get(worker_id, 0.0)
        """
    ...
    def generate_worker_report(self, worker_id):
        ...
        try:
            ...
            weekend_count = sum(
                1 for date in assignments if self.scheduler.date_utils.is_weekend_day(date, self.scheduler.holidays)
            )
            ...
            if abs(target_diff) > 1:
                report.append(
                    f"  - Worker is {'over-assigned' if target_diff > 0 else 'under-assigned'} "
            ...
            logging.error(f"Error generating report for worker {worker_id}: {e!s}", exc_info=True)
