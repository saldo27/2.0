...
from reportlab.lib.pagesizes import A4, landscape  # Keep A4 if needed elsewhere
from reportlab.lib.styles import ParagraphStyle, getSampleStyleSheet
from reportlab.lib.units import cm, inch  # Use cm for better control maybe
...
class PDFExporter:
    ...
    def export_summary_pdf(self, stats_data):  # Takes the whole stats dictionary now
        ...
        try:
            ...
            if not workers_stats:
                ...
                for worker_id, stats in sorted(worker_items, key=numeric_sort_key):
                    ...
                    weekdays_str = "<b>Weekdays:</b> " + " ".join(
                        f"{weekdays_short[i]}:{weekday_counts.get(i, 0)}" for i in range(7)
                    )
                    ...
                    posts_str = "<b>Posts:</b> " + " ".join(
                        f"P{post + 1}:{count}" for post, count in sorted(post_counts.items())
                    )
                    ...
                    if worker_shifts:
                        ...
                        shifts_table = Table(
                            shifts_data, colWidths=[2.5 * cm, 1.5 * cm, 1.5 * cm, 1.5 * cm], repeatRows=1
                        )
            ...
            logging.error(f"Failed to export GLOBAL summary PDF: {e!s}", exc_info=True)
    ...
    def export_monthly_calendar(self, year, month, filename=None):
        ...
        # Add title
        title_style = ParagraphStyle("CustomTitle", parent=self.styles["Heading1"], fontSize=16, spaceAfter=30)
        title = Paragraph(f"Schedule for {datetime(year, month, 1).strftime('%B %Y')}", title_style)
        story.append(title)
    ...
    def export_all_months_calendar(self, filename=None):
        ...
        try:
            ...
            while current_date <= end_month:
                ...
                month_title = Paragraph(f" {current_date.strftime('%B %Y')}", title_style)
            ...
            logging.error(f"Failed to export multi-month calendar PDF: {e!s}", exc_info=True)
            raise
    ...
    def export_worker_statistics(self, filename=None):
        ...
        for worker in self.workers_data:
            ...
            assignments = {date for date, workers in self.schedule.items() if worker_id in workers}
        ...
        for worker_id, stats in worker_stats_cache.items():
            ...
            worker_title = Paragraph(f"Worker {worker_id}", self.styles["Heading2"])
            story.append(worker_title)
            ...
            details_text = Paragraph("<br/>".join(details_parts), self.styles["Normal"])
