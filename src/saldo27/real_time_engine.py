...
class RealTimeEngine:
    ...
    def assign_worker_real_time(
        ...
        try:
            ...
            return RealTimeOperationResult(
                success=False, message=f"Assignment failed: {e!s}", operation_id=operation_id
            )
        finally:
            with self._operation_lock:
    ...
    def unassign_worker_real_time(
        ...
        try:
            ...
            update_result = self.incremental_updater.unassign_worker_from_shift(shift_date, post_index, user_id)
            ...
            if update_result.success:
                ...
                return RealTimeOperationResult(
                    success=True, message=update_result.message, operation_id=operation_id, suggestions=suggestions
                )
            else:
                return RealTimeOperationResult(success=False, message=update_result.message, operation_id=operation_id)

        except Exception as e:
            logging.error(f"Error in real-time unassignment: {e}")
            return RealTimeOperationResult(
                success=False, message=f"Unassignment failed: {e!s}", operation_id=operation_id
            )
        finally:
            with self._operation_lock:
                if operation_id in self._active_operations:
    ...
    def swap_workers_real_time(
        ...
        try:
            ...
            return RealTimeOperationResult(success=False, message=f"Swap failed: {e!s}", operation_id=operation_id)
        finally:
            with self._operation_lock:
                if operation_id in self._active_operations:
                    end_time = datetime.now()
    ...
    def validate_schedule_real_time(self, quick_check: bool = False) -> RealTimeOperationResult:
        """
        ...
        try:
            with self._operation_lock:
                self._active_operations[operation_id] = {"type": "validation", "start_time": datetime.now()}
            ...
            return RealTimeOperationResult(
                success=False, message=f"Validation failed: {e!s}", operation_id=operation_id
            )
        finally:
            with self._operation_lock:
                if operation_id in self._active_operations:
    ...
    def undo_last_change(self, user_id: str | None = None) -> RealTimeOperationResult:
        """
        ...
        try:
            if not self.change_tracker.can_undo():
                return RealTimeOperationResult(
                    success=False, message="No changes available to undo", operation_id=operation_id
                )
            ...
            if undone_change:
                return RealTimeOperationResult(
                    success=True, message=f"Undid: {undone_change.description}", operation_id=operation_id
                )
            else:
                return RealTimeOperationResult(
                    success=False, message="Failed to undo change", operation_id=operation_id
                )

        except Exception as e:
            logging.error(f"Error in undo operation: {e}")
            return RealTimeOperationResult(success=False, message=f"Undo failed: {e!s}", operation_id=operation_id)

    def redo_last_change(self, user_id: str | None = None) -> RealTimeOperationResult:
        """
        ...
        try:
            if not self.change_tracker.can_redo():
                return RealTimeOperationResult(
                    success=False, message="No changes available to redo", operation_id=operation_id
                )
            ...
            if redone_change:
                return RealTimeOperationResult(
                    success=True, message=f"Redid: {redone_change.description}", operation_id=operation_id
                )
            else:
                return RealTimeOperationResult(
                    success=False, message="Failed to redo change", operation_id=operation_id
                )

        except Exception as e:
            logging.error(f"Error in redo operation: {e}")
            return RealTimeOperationResult(success=False, message=f"Redo failed: {e!s}", operation_id=operation_id)
    ...
    def get_real_time_analytics(self) -> dict[str, Any]:
        ...
        try:
            ...
            workloads = {
                worker_id: len(assignments) for worker_id, assignments in self.scheduler.worker_assignments.items()
            }
            avg_workload = sum(workloads.values()) / len(workloads) if workloads else 0

            return {
                "timestamp": datetime.now().isoformat(),
                "active_operations": {"count": active_ops, "types": active_op_types},
                "schedule_metrics": {
        ...
        try:
            ...
            if empty_slots and len(suggestions) < limit:
                ...
                if available_workers:
                    suggestions.append(
                        f"Fill empty slot on {date.strftime('%Y-%m-%d')} post {post + 1}: {available_workers[0]}"
                    )
    ...
    def _get_optimization_suggestions(self, shift_date: datetime) -> list[str]:
        ...
        try:
            ...
            if shift_date in self.scheduler.schedule:
                empty_posts = [i for i, worker in enumerate(self.scheduler.schedule[shift_date]) if worker is None]
