...
class ShiftToleranceValidator:
    ...
    def validate_worker_shift_count(self, worker_id: str, is_weekend_only: bool = False) -> dict[str, Any]:
        ...
        worker = next((w for w in self.workers_data if w["id"] == worker_id), None)
        if not worker:
            return {"valid": False, "error": f"Worker {worker_id} not found"}
        ...
        if not is_weekend_only:
            ...
            if mandatory_str:
                try:
                    ...
                    mandatory_in_period = [
                        d for d in mandatory_dates if self.scheduler.start_date <= d <= self.scheduler.end_date
                    ]
        ...
        min_shifts, max_shifts = self.calculate_tolerance_bounds(
            target_shifts, is_weekend=is_weekend_only, worker_id=worker_id
        )
                    ...
                    suggestions.append(
                        {
                            ...
                            "is_weekend_only": is_weekend_only,
                            "priority": abs(over_worker["deviation_percentage"])
                            + abs(under_worker["deviation_percentage"]),
                        }
                    )
    ...
    def log_tolerance_report(self) -> None:
        ...
        if general_suggestions:
            ...
            for suggestion in general_suggestions[:5]:  # Top 5
                logging.info(
                    f"  Transfer {suggestion['shifts_to_transfer']} shifts from "
        ...
        if weekend_suggestions:
            ...
            for suggestion in weekend_suggestions[:5]:  # Top 5
                logging.info(
                    f"  Transfer {suggestion['shifts_to_transfer']} weekend shifts from "
                    f"{suggestion['from_worker']} to {suggestion['to_worker']}"
                )
