...
class IncrementalUpdater:
    ...
    def assign_worker_to_shift(
        self, worker_id: str, shift_date: datetime, post_index: int, user_id: str | None = None, force: bool = False
    ) -> UpdateResult:
        """
        ...
        try:
            ...
            return UpdateResult(True, f"Successfully assigned {worker_id}", rollback_data=rollback_data)

        except Exception as e:
            logging.error(f"Error assigning worker: {e}")
            return UpdateResult(False, f"Assignment failed: {e!s}")

    def unassign_worker_from_shift(
        ...
        try:
            ...
            if hasattr(self.scheduler, "schedule_builder") and self.scheduler.schedule_builder:
                if self.scheduler.schedule_builder._is_mandatory(current_worker, shift_date):
                    ...
                    return UpdateResult(
                        False,
                        f"Cannot unassign: Worker {current_worker} has a MANDATORY assignment on {shift_date.strftime('%d-%m-%Y')}",
                    )
            ...
            return UpdateResult(True, f"Successfully unassigned {current_worker}", rollback_data=rollback_data)

        except Exception as e:
            logging.error(f"Error unassigning worker: {e}")
            return UpdateResult(False, f"Unassignment failed: {e!s}")
        ...
        try:
            ...
            if not force:
                constraint_result = self._check_swap_constraints(
                    worker1, shift_date1, post_index1, worker2, shift_date2, post_index2
                )
                if not constraint_result.success:
                    return constraint_result
            ...
            return UpdateResult(
                True, f"Successfully swapped {worker1 or 'empty'} and {worker2 or 'empty'}", rollback_data=rollback_data
            )

        except Exception as e:
            logging.error(f"Error swapping workers: {e}")
            return UpdateResult(False, f"Swap failed: {e!s}")

    def bulk_update(self, updates: list[dict[str, Any]], user_id: str | None = None) -> UpdateResult:
        """
        ...
        try:
            ...
            for i, update in enumerate(updates):
                ...
                if operation == "assign":
                    ...
                    result = self.unassign_worker_from_shift(
                        datetime.fromisoformat(update["shift_date"]), update["post_index"], user_id=user_id
                    )
                elif operation == "swap":
                ...
                if result.success:
                    ...
                    rollback_operations.append(
                        {"operation": operation, "rollback_data": result.rollback_data, "index": i}
                    )
                else:
            ...
            return UpdateResult(False, f"Bulk update failed: {e!s}")
    ...
    def _check_assignment_constraints(self, worker_id: str, shift_date: datetime, post_index: int) -> UpdateResult:
        ...
        try:
            ...
            can_assign, reason = self.scheduler.constraint_checker._check_constraints(worker_id, shift_date)

            if not can_assign:
                suggestions = self._generate_assignment_suggestions(shift_date, post_index)
                return UpdateResult(
                    False, f"Constraint violation: {reason}", conflicts=[reason], suggestions=suggestions
                )
            ...
            return UpdateResult(False, f"Constraint check failed: {e!s}")

    def _check_swap_constraints(
        self,
        worker1: str | None,
        shift_date1: datetime,
        post_index1: int,
        ...
        if not constraints_ok:
            return UpdateResult(
                False, "Cannot swap: one or more assignments are MANDATORY and cannot be moved", conflicts=conflicts
            )
        ...
        if worker1:
            ...
            if shift_date2 in self.scheduler.schedule:
                assigned_workers_shift2 = [
                    w for i, w in enumerate(self.scheduler.schedule[shift_date2]) if w is not None and i != post_index2
                ]
        ...
        if worker2:
            ...
            if shift_date1 in self.scheduler.schedule:
                assigned_workers_shift1 = [
                    w for i, w in enumerate(self.scheduler.schedule[shift_date1]) if w is not None and i != post_index1
                ]
    ...
    def _create_unassignment_rollback_data(
        self, shift_date: datetime, post_index: int, worker_id: str
    ) -> dict[str, Any]:
    ...
    def _update_assignment_tracking(
        self, worker_id: str, shift_date: datetime, post_index: int, previous_worker: str | None
    ):
