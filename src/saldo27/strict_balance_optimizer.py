...
class StrictBalanceOptimizer:
    ...
    def __init__(self, scheduler, schedule_builder):
        ...
        # Estadísticas
        self.stats = {"swaps_performed": 0, "workers_balanced": 0, "max_deviation_before": 0, "max_deviation_after": 0}
    ...
    def optimize_balance(self, max_iterations: int = 500, target_tolerance: int = 1) -> bool:
        ...
        logging.info(
            f"Workers outside tolerance: {initial_analysis['workers_outside_tolerance']} → {final_analysis['workers_outside_tolerance']}"
        )
            ...
            for under_id, under_dev in underloaded[:5]:  # Top 5 subcargados
                ...
                for date in over_assignments:
                    ...
                    if hasattr(self.builder, "_get_expected_monthly_target"):
                        expected_monthly = self.builder._get_expected_monthly_target(
                            worker_under, date.year, date.month
                        )
                        shifts_this_month = sum(
                            1
                            for d in self.worker_assignments.get(under_id, set())
                    ...
                    if hasattr(self.builder, "_is_weekend_or_holiday") and self.builder._is_weekend_or_holiday(date):
                        if hasattr(self.builder, "_would_exceed_weekend_limit_simulated"):
                            if self.builder._would_exceed_weekend_limit_simulated(
                                under_id, date, self.worker_assignments
                            ):
        ...
        for over_id, over_dev in overloaded[:5]:  # Top 5 sobrecargados
            ...
            for date_a in over_dates[:10]:  # Limitar para rendimiento
                ...
                for worker_c in self.workers_data:
                    ...
                    if hasattr(self.builder, "_get_expected_monthly_target"):
                        expected_monthly = self.builder._get_expected_monthly_target(
                            worker_c, date_a.year, date_a.month
                        )
                        shifts_c_month_a = sum(
                            1
                            for d in self.worker_assignments.get(c_id, set())
                    ...
                    if (
                        not skip_c_take
                        and hasattr(self.builder, "_is_weekend_or_holiday")
                        and self.builder._is_weekend_or_holiday(date_a)
                    ):
                        if hasattr(self.builder, "_would_exceed_weekend_limit_simulated"):
                            if self.builder._would_exceed_weekend_limit_simulated(
                                c_id, date_a, self.worker_assignments
                            ):
                                skip_c_take = True
                    ...
                    for date_c in c_dates[:10]:
                        ...
                        for under_id, under_dev in underloaded[:5]:
                            ...
                            score_b_take_c = self.builder._calculate_worker_score(
                                worker_b, date_c, post_c, relaxation_level=0
                            )
                            ...
                            if hasattr(self.builder, "_get_expected_monthly_target"):
                                expected_monthly_b = self.builder._get_expected_monthly_target(
                                    worker_b, date_c.year, date_c.month
                                )
                            ...
                            if (
                                not skip_b_take
                                and hasattr(self.builder, "_is_weekend_or_holiday")
                                and self.builder._is_weekend_or_holiday(date_c)
                            ):
                                if hasattr(self.builder, "_would_exceed_weekend_limit_simulated"):
                                    if self.builder._would_exceed_weekend_limit_simulated(
                                        under_id, date_c, self.worker_assignments
                                    ):
                                        skip_b_take = True
                            ...
                            try:
                                ...
                                over_target = next(
                                    (w["target_shifts"] for w in self.workers_data if w["id"] == over_id), 0
                                )
                                under_target = next(
                                    (w["target_shifts"] for w in self.workers_data if w["id"] == under_id), 0
                                )
                                ...
                                if over_improved and under_improved and c_stable:
                                    logging.info(
                                        f"  ✅ Three-way swap: {over_id}({date_a.strftime('%m-%d')})→{c_id}→{under_id}({date_c.strftime('%m-%d')})"
                                    )
                                    logging.debug(
                                        f"      A:{over_id} {over_dev:+d}→{over_new - over_target:+d}, "
                ...
                for under_id, under_dev in underloaded[:3]:
                    ...
                    if score > float("-inf"):
                        ...
                        if (
                            not skip_assign
                            and hasattr(self.builder, "_is_weekend_or_holiday")
                            and self.builder._is_weekend_or_holiday(date)
                        ):
                            if hasattr(self.builder, "_would_exceed_weekend_limit_simulated"):
                                if self.builder._would_exceed_weekend_limit_simulated(
                                    under_id, date, self.worker_assignments
                        ...
                        logging.info(
                            f"  ✅ Reassignment: {over_id} removed, {under_id} assigned on {date.strftime('%Y-%m-%d')}"
                        )
                        return True
            ...
            for under_id, under_dev in underloaded[:3]:
                ...
                for date in over_assignments:
                    ...
                    if score > float("-inf"):
                        ...
                        counts_ok = post_over_count == pre_over_count - 1 and post_under_count == pre_under_count + 1
                        if not counts_ok:
                            logging.error(
                ...
                for c_id, worker_c, c_dev, c_target in potential_intermediaries[:10]:
                    ...
                    for date_c in c_dates[:15]:
                        ...
                        for under_id, under_dev in underloaded[:8]:
                            ...
                            score_b_take_c = self.builder._calculate_worker_score(
                                worker_b, date_c, post_c, relaxation_level=1
                            )
                            if score_b_take_c == float("-inf"):
                                continue
                            ...
                            skip_b_take = False
                            if hasattr(self.builder, "_get_expected_monthly_target"):
                                _b_exp_m = self.builder._get_expected_monthly_target(
                                    worker_b, date_c.year, date_c.month
                                )
                                _b_month_c = sum(
                                    1
                            ...
                            try:
                                ...
                                if new_total_dev < old_total_dev:
                                    logging.info(
                                        f"  ✅ Aggressive three-way: {over_id}({date_a.strftime('%m-%d')})→{c_id}→{under_id}({date_c.strftime('%m-%d')})"
                                    )
    ...
    def _find_swap_chain(
        self, start_id: str, start_date, start_post: int, underloaded: list, max_depth: int = 4
    ) -> list | None:
        """
                ...
                for post, assigned in enumerate(slots):
                    if assigned is None:
                        ...
                        if score > float("-inf"):
                            ...
                            self.scheduler._update_tracking_data(under_id, date, post, removing=False)
                            logging.info(
                                f"  ✅ Forced fill: {under_id} assigned empty slot on {date.strftime('%Y-%m-%d')}"
                            )
                            return True
                ...
                for date in mid_dates[:10]:
                    ...
                    for under_id, under_dev in underloaded[:5]:
                        ...
                        if score > float("-inf"):
                            ...
                            logging.info(
                                f"  ✅ Forced redistribution: {mid_id}(+{mid_dev}) → {under_id}({under_dev}) on {date.strftime('%Y-%m-%d')}"
                            )
                            return True
