...
class ProgressMonitor:
    ...
    def track_iteration_progress(
        self, iteration: int, operation_results: dict[str, Any], current_score: float, metrics_data: dict[str, Any]
    ) -> dict[str, Any]:
        """
        ...
        try:
            ...
            if len(self.iteration_times) > 0:
                avg_time_per_iteration = sum(
                    (t.total_seconds() if hasattr(t, "total_seconds") else t) for t in self.iteration_times
    ...
    def _display_progress_bar(self, iteration: int, current_score: float, progress_data: dict[str, Any]):
        ...
        try:
            ...
            logging.info(
                f"⏱️  Tiempo: {elapsed_str}{eta_str} | "
    ...
    def _display_detailed_metrics(self, progress_data: dict[str, Any], operation_results: dict[str, Any]):
        ...
        try:
            ...
            if metrics:
                logging.info("📈 Métricas actuales:")
    ...
    def display_optimization_summary(self, final_iteration: int, final_score: float, termination_reason: str):
        ...
        try:
            ...
            significant_improvements = sum(
                1 for data in self.iteration_data if data.get("is_significant_improvement", False)
            )
