...
class PerformanceCache:
    ...
    def _generate_key(self, func_name: str, args: tuple, kwargs: dict) -> str:
        ...
        try:
            ...
            hashable_kwargs = self._make_hashable(sorted(kwargs.items()) if kwargs else {})

            key_data = {"func": func_name, "args": hashable_args, "kwargs": hashable_kwargs}

            # Use repr-based hashing — avoids pickle failures with
            # unpicklable objects (RLock, threading primitives, etc.)
            fallback_key = f"{func_name}_{hashable_args!r}_{hashable_kwargs!r}"
    ...
    def _evict_expired_entries(self) -> None:
        ...
        expired_keys = [
            key for key, entry in self._cache.items() if "expires_at" in entry and current_time > entry["expires_at"]
        ]
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        with self._lock:
            ...
            expired_count = sum(
                1 for entry in self._cache.values() if "expires_at" in entry and current_time > entry["expires_at"]
            )
...
class PerformanceMonitor:
    ...
    def get_metric_stats(self, metric_name: str, since: datetime | None = None) -> dict[str, float]:
