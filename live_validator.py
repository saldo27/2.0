...
class LiveValidator:
        ...
        try:
            ...
            return [
                ValidationResult(
                    is_valid=False,
                    severity=ValidationSeverity.ERROR,
                    message=f"Schedule validation failed: {e!s}",
                    constraint_type="system_error",
                )
            ]
    ...
    def _check_data_synchronization(self) -> ValidationResult:
        ...
        try:
            ...
            return ValidationResult(
                is_valid=False,
                severity=ValidationSeverity.ERROR,
                message=f"Synchronization check failed: {e!s}",
                constraint_type="data_synchronization",
            )

    def detect_conflicts(self, date_range: tuple[datetime, datetime] | None = None) -> list[ConflictInfo]:
        """
        ...
        for worker_id, workload in workloads.items():
            if abs(workload - avg_workload) > threshold:
                severity = (
                    ...
                    if abs(workload - avg_workload) < threshold * 2
            ...
            if deviation > avg_workload * 0.5:  # 50% deviation threshold
                conflicts.append(
                    ConflictInfo(
                        ...
                        workers_involved=[worker_id],
                        dates_involved=[],
                        resolution_suggestions=[f"Redistribute shifts to balance workload for worker {worker_id}"],
                    )
                )
    ...
    def _calculate_assignment_priority(self, worker_id: str, shift_date: datetime) -> float:
        ...
        avg_workload = sum(len(assignments) for assignments in self.scheduler.worker_assignments.values()) / len(
